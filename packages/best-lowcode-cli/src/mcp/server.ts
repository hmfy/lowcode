  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      },
      {
        ...
        description: '将页面需求转换为受控的 AgentTask，不写入文件。',
        inputSchema: {
          type: 'object',
          properties: {
            ...
            semantic: {
              type: 'string',
              enum: ['codex'],
              description: '使用本地 Codex 做受限语义选择（默认启用）'
            }
          },
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (request.params.name) {
      ...
      case 'best_prepare_task': {
        ...
        const semantic = stringArg(args, 'semantic')
        return taskRequest
          ? textResult(
              await (await getService(args)).prepareTask(taskRequest, semantic === 'codex' ? { semantic } : {})
            )
